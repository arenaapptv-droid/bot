...
ADMIN_ID = 8266981888                    # ID حسابك في تلجرام
API_HOST = "127.0.0.1"
API_PORT = 8000
API_KEY = "CHANGE_THIS_SECRET_KEY"       # مفتاح لحماية API
...
async def monitor_single_stream(stream_id: int, proc: asyncio.subprocess.Process):
    """مراقبة عملية واحدة وإعادة تشغيلها إذا توقفت بشكل غير متوقع"""
    ...
    if stream_id in streams and streams[stream_id].get("status") == "running":
        # إعادة تشغيل تلقائي مع محاولات محدودة
        count = retry_counter.get(stream_id, 0) + 1
        retry_counter[stream_id] = count
        delay = min(30, count * 3)  # تأخير تصاعدي حتى 30 ثانية
        if count <= 10:
...
async def cmd_start(message: types.Message):
    ...
    for sid, s in streams.items():
        ...
        await message.answer(
            f"🎬 *{s['name']}*\n"
            f"📡 الحالة: `{s['status']}`\n"
            f"⏱️ بدأ: {time.strftime('%H:%M:%S', time.localtime(s['started_at'])) if s['started_at'] else '—'}",
            reply_markup=kb,
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    async def send_api_request(endpoint: str):
        ...
        try:
            ...
            return {"success": False, "message": f"API error: {str(e)}", "status": "error"}

    import aiohttp  # نضعه داخل الدالة لتجنب استيراده في البداية

    if action == "start":
        res = await send_api_request("start")
        await callback.message.reply(res.get("message", "Unknown"))
    elif action == "stop":
        ...
        res = await send_api_request("status")
...
# ═══════════════════════════════════════════
# تشغيل الخادمين معاً (FastAPI و Bot) في حلقة asyncio واحدة
# ═══════════════════════════════════════════
async def run_api():
...
if __name__ == "__main__":
    ...
    # إنشاء بيانات افتراضية للعرض إذا كانت فارغة
    if not streams:
        streams = {
            1: {
                "name": "قناة رياضية",
